<template>
  <div class="drip-slider">
    <div class="drip-swiper" :style="{height: xheight}">
      <slot></slot>

      <div
      class="drip-swiper-item"
      v-for="(item, index) in list"
      :key=index
      @click="clickListItem(item)"
      :data-index="index">
        <a href="javascript:">
          <div class="drip-img" :style="{backgroundImage: buildBackgroundUrl(item)}"></div>
          <p class="drip-swiper-desc" v-if="showDescMask">{{ item.title }}</p>
        </a>
      </div>

      <div
      v-if="listTwoLoopItem.length > 0"
      class="drip-swiper-item drip-swiper-item-clone"
    ...
    <div :class="[dotsClass, 'drip-indicator', `drip-indicator-${dotsPosition}`]" v-show="showDots">
      <a href="javascript:" v-for="(key, index) in length" :key="index">
        <i class="drip-icon-dot" :class="{'active': key - 1 === current}"></i>
      </a>
    </div>
  </div>
</template>
...
.@{pre}-slider {
  ...
  > .@{pre}-indicator-left-down {
    left: 15px;
    right: auto;
    bottom: -30px;
  }

  > .@{pre}-indicator-right-down {
    bottom: -30px;
  }

  > .@{pre}-indicator-center-down {
    right: 50%;
    transform: translateX(50%);
    bottom: -30px;
  }
